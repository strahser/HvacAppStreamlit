...
from IFC.tools import ifchelper
import json
import ifcopenshell
...
def initialise_debug_props(force=False):
	if not "BIMDebugProperties" in st.session_state:
...
def get_object_data(fromId=None):
	...
	if st.session_state.BIMDebugProperties:
		initialise_debug_props(force=True)
		step_id = 0
		if fromId:
			step_id = int(fromId)
		else:
			step_id = int(st.session_state.object_id) if st.session_state.object_id else 0
		...
		element = st.session_state.ifc_file.by_id(step_id)
...
def edit_object_data(object_id, attribute):
	"""get selected data"""
	try:
		entity = st.session_state.ifc_file.by_id(object_id)
		st.write("selected items:", getattr(entity, attribute))
...
def viewer_execute():
	...
	if "ifc_file" in st.session_state and st.session_state["ifc_file"]:
		tab1, tab2, tab3 = st.tabs(["🎮 IFC.js Viewer", "🧮 Properties", "🩺 Debugger"])
		...
		st.subheader("Step 1: Load a file from the Home Page")
