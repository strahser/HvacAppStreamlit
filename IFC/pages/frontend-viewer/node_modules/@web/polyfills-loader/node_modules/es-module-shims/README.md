...
This error is important - it means that the native browser loader didn't viewer_execute any of the code at all because this error happens
...
In the above, the native browser loader without import maps support will viewer_execute the above module, but fail the dynamic import.
...
/**
 * 
 * @param {string} aURL a string representing the URL of the module script the worker will viewer_execute.
 * @returns {string} The string representing the URL of the script the worker will viewer_execute.
 */
...
For example in the following, only the listed `app.js` and `dep.js` modules will be able to viewer_execute with the provided integrity:
...
Because of the extra processing done by ES Module Shims it is possible for static module scripts to viewer_execute after the `DOMContentLoaded` or `readystatechange` events they expect, which can cause missed attachment.
