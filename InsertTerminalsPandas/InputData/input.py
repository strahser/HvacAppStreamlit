...
SystemFlowAndNameData = namedtuple('SystemProperty', ['system_flow', 'system_name'])
...
class InputDataDF:
	...
	def create_config_data(self):
		...
		self.system_dictionary = {
			records['system_type']: SystemFlowAndNameData(records['system_flow'], records['system_name']) for records in
			self.config
		}
