...
from Networks.CalculationNetwork.Network_engineer import *
...
class NetworkAddPressure:
    def __init__(self,
        ...
        self.df = df_branch
        self.branch_columns = ['S_ID', 'm_idx',
                            'sys_flow_column', 'pcx', 'pcy', ]
        self.route_columns = ['m_idx', 'shift_idx',
                            'sum_column', 'x_intersect', 'y_intersect', ]
    ...
    def _get_columns_names(self, root_type):
        if root_type == "branch":
            return self.branch_columns
        else:
            return self.route_columns

    def get_old_columns_names(self, root_type):
        old_columns = [getattr(self.network, val)
                    for val in self._get_columns_names(root_type)]
        return old_columns

    def _get_rename_columns_names(self, root_type):
        self.new_columns = ['from', 'to', 'power', 'from_x', 'from_y', ]
        old_columns = [getattr(self.network, val)
                    for val in self._get_columns_names(root_type)]
        rename_columns = dict(
            zip(old_columns, self.new_columns))
    ...
    def apply_to_coordinate_column(self, root_type):
        ...
        if root_type == "branch":
            df = df.assign(
                to_x=self.df[self.network.x_intersect]
            )
            df = df.assign(
                to_y=self.df[self.network.y_intersect]
            )
        else:
            df = df.assign(
                to_x=self.df[self.network.x_intersect].shift(-1)
            )
            df = df.assign(
                to_y=self.df[self.network.y_intersect].shift(-1)
            )
        return df
    ...
    def rename_df_columns(self, root_type):
        ...
        new_columns = ['from', 'to', 'power', 'from_x', 'from_y', ]
        rename_columns = dict(zip(old_columns, new_columns))
    ...
    def get_pressure_data_column(self, power: float, network_branch_type: str):
        ...
        """
        seting_builder = SettingBuilder(
            self.excel_loader, power, network_branch_type)
    ...
    def _apply_pressure_data_columns(self, root_type):
        ...
        df["seting_builder"] = df.apply(lambda x: SettingBuilder(
            self.excel_loader, x['power'], root_type), axis=1)
        df["flow"] = df.apply(lambda x: x["seting_builder"].get_flow(), axis=1)
        df["k_local_pressure"] = df.apply(
            lambda x: x["seting_builder"].get_k_local_pressure(), axis=1)
        df["diameter"] = df.apply(
            lambda x: x["seting_builder"].choose_standart_diameter(), axis=1)
        df["velocity"] = df.apply(
            lambda x: x["seting_builder"].drop_presuer.get_velocity(), axis=1)
        df["line_pressure"] = df.apply(
            lambda x: x["seting_builder"].drop_presuer.get_presure_line_drop(), axis=1)
        df["dinamic_pressure"] = df.apply(lambda x: x["seting_builder"].
                                        drop_presuer.get_full_dynamic_drop_pressure(x["k_local_pressure"]), axis=1)
        df = df.drop(columns='seting_builder')
        df = df.assign(
            full_line_pressure=df['distance']*df['line_pressure'])
        df = df.assign(
            full_pressure=df['full_line_pressure']+df['dinamic_pressure'])
    ...
    def _get_filtred_df(self,df):
        filtred_columns = ['S_Name','from','to','distance','power', 'flow','diameter','velocity', 'line_pressure', 'dinamic_pressure','full_pressure',]
        df = df.filter(filtred_columns)
        return df
    ...
    def drop_center_rows_dupblicates(self):
        ...
        # df = df.drop_duplicates(['from','to'],keep ='first')
        return df
...
class GetLongRoute:
    ...
    def get_long_df(self):
        ...
        # empty_row = {col: "" for col in df_filter.columns}
        # empty_row["full_pressure"] = df_filter["full_pressure"].sum()
        # df_filter = df_filter.append(empty_row, ignore_index=True)
        return df_filter
...
class NetworkGraph:
    ...
    def create_network(self, from_column, to_column, value_column):
        ...
        """
        for sourse, end_p, weght, in zip(self.df[from_column], self.df[to_column], self.df[value_column]):
