...
class SetColor:
    def __init__(self, df: pd.DataFrame, color_filter_name:str, idx:str='S_ID') -> None:
        """set color to new df column by filter 

        Args:
            df (pd.DataFrame): df with filter column 
            color_filter_name (str): name of df column
            idx (str, optional): _description_. Defaults to 'S_ID'.
        """
        ...
        self.color_circle = cycle(
            'orange,indigo,lightpink,darkblue,firebrick,orchid,yellowgreen,goldenrod,blue,brown,red,coral,darkmagenta,green,cyan,deeppink'.split(','))
    ...
    def set_color_by_category(self):
        ...
        category_color = {cat: next(self.color_circle)
                        for en, cat in enumerate(category)}
        new_df = pd.DataFrame(category_color.items(), columns=[
                            self.color_filter_name, "color"])
        return new_df
