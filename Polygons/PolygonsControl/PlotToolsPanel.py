...
class PlotToolsPanel:
	def __init__(self,
		...
		self.show_plots = st.sidebar.checkbox("Show Plots", key=f"{self.key} show_plots")

	def create_plot_tools_panel(self, selected_table_value: list):
		self._create_polygons_config()
		# if self._which_level_plots_create():
		self._create_plots(selected_table_value)
	...
	def _create_plots(self, selected_table_value: list):
		"""
...
def show_selected_levels(level_list: list[str]):
