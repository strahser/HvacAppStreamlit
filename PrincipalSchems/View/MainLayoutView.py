...
class LayoutViewContextData:
	def __init__(
			self,
			system_and_flow_layout_view: SystemAndFlowLayoutView,
			static_layout: StaticLayoutView,
	) -> None:
		...
		self.plote_titel = static_layout.plot_title
		...
		self.vertical_direction_list: str = (
			system_and_flow_layout_view.system_and_flow_view.vertical_direction_list
		)
		self.horizontal_direction_list: str = (
			system_and_flow_layout_view.system_and_flow_view.horizontal_direction_list
		)
		self.main_columns: list = system_and_flow_layout_view.main_columns
		self.system_columns: list[
			str
		] = system_and_flow_layout_view.system_and_flow_view.system_columns
		self.flow_columns: list[
			str
		] = system_and_flow_layout_view.system_and_flow_view.flow_columns


class DynamicLayoutViewContextData:
	def __init__(self, dynamic_layout_view: DynamicLayoutView) -> None:
		"""system,flow,line direction"""
		self.base_property_wiget = dynamic_layout_view.system_view
		# distance,color
		self.level_property_wiget = dynamic_layout_view.level_view
		# system and flow
...
class DynamicWidgetsViewContextData:
	...
	def _get_level_list_value(self):
		level_list_value = [
			getattr(
				self.dynamic_layout_context_data.level_property_wiget,
				f"level_label_{val}",
			)
	...
	def _get_equipment_symbol_list(self):
	...
	def calculate(self):
		self.level_list_value = self._get_level_list_value()
		self.legend_group = self._get_legend_group()
		self.flow_list_value = self._get_flow_list_value()
		self.equipment_symbol_list = self._get_equipment_symbol_list()
